...
<center>     <h3>لا توجد أي وثائق</h3>          </center>
@else
        ...
        @foreach($b as $a)
          ...
          <div class="img"style="background-image: url(kiddos/images/course-1.png);"></div>
						<div class="text bg-light p-4">
							<h3><a href="{{$a->file}}">{{$a->titre}} </a></h3>
              <br>
              @if ($a->etat == 'admin'  )
              <h7><a>  {{$a->nom}}  :  انزل من قبل مؤسس الموقع  </a></h7>
              @else 
              <h7><a>  {{$a->nom}}  : انزل من قبل المعلم  </a></h7>
              @endif
						</div>
					</div>
          @endforeach
